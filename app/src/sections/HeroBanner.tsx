    ...
    <div className="relative h-[50vh] md:h-[60vh] min-h-[400px] overflow-hidden">
      ...
      <div className="absolute bottom-0 left-0 right-0 z-20 p-6 md:p-12 max-w-3xl">
        ...
        <div className="flex items-center gap-3 mt-3 md:mt-4">
          <button onClick={() => navigate(`/watch/${item.stream_type || 'movie'}/${item.id}/${slugify(item.name || '')}`)} className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-[#0099ff] text-white font-semibold text-sm hover:bg-[#0088ee] transition-colors">
            <Play className="w-4 h-4 fill-white" />İzle
          </button>
          <button onClick={() => navigate(`/detail/${item.stream_type || 'movie'}/${item.id}/${slugify(item.name || '')}`)} className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-white/10 text-white font-semibold text-sm hover:bg-white/20 transition-colors backdrop-blur-sm">
