...
export default function GamesScreen() {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      if (menuRef.current && !menuRef.current.contains(e.target as Node) && source !== 'gam-onl') setMenuOpen(false)
    }
      ...
      <div ref={playerRef} className="flex-1 flex flex-col min-h-0">
        <div className="flex items-center gap-2 px-3 py-2 bg-black/80 backdrop-blur-sm z-30 shrink-0 border-b border-white/5">
          <div className="flex items-center gap-1 bg-white/5 rounded-lg p-0.5 shrink-0">
            ...
            >Klasik Windows</button>
            <button onClick={() => switchSource('gam-onl')}
              className={`px-2 py-1 text-[10px] rounded-md transition-colors ${source === 'gam-onl' ? 'bg-[#0099ff] text-white' : 'text-gray-400 hover:text-white'}`}
            >Nintendo Atari</button>
          </div>
          {source !== 'gam-onl' && (
            <div ref={menuRef} className="relative">
              <button
                onClick={() => setMenuOpen(p => !p)}
                className="flex items-center gap-1.5 bg-white/10 text-white text-xs rounded-lg px-2 py-1.5 border border-white/10 outline-none cursor-pointer max-w-[140px] sm:max-w-[180px]"
              >
