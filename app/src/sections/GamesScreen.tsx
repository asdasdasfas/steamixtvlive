      ...
      <div ref={playerRef} className="flex-1 flex flex-col min-h-0">
        <div className="flex items-center gap-2 px-3 py-2 bg-black/80 backdrop-blur-sm z-30 shrink-0 border-b border-white/5">
          <select
            ...
            className="bg-white/10 text-white text-xs rounded-lg px-2 py-1.5 border border-white/10 outline-none appearance-none cursor-pointer max-w-[180px] sm:max-w-[250px]"
            size={1}
          >
            {cats.map(cat => (
              <optgroup key={cat} label={cat}>
                {gamesByCat[cat].map(g => (
                  <option key={g.slug} value={g.slug} className="bg-gray-900">{g.name}</option>
                ))}
              </optgroup>
            ))}
          </select>
          <span className="text-sm font-semibold text-white flex-1 truncate ml-1">{game?.name}</span>
          <button onClick={toggleFullscreen}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/10 text-white text-xs hover:bg-white/20 transition-colors shrink-0">
